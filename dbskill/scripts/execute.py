"""
写操作脚本（Direct / API 双模式的对外入口）。

当前实现 Direct 模式：
- 读取 config.yaml；
- 基于 db_alias/default_db 选择数据库；
- 按 permission 控制写权限，区分 write/full；
- 限定仅允许 INSERT/UPDATE/DELETE，且 DELETE 仅 full 允许；
- 执行参数化写操作；
- 写入本地审计日志（如启用）。
...
from dbskill.api_client import call_execute as api_call_execute
...
def run_execute(
    ...
    :param sql: 参数化 SQL 模板（仅允许 INSERT/UPDATE/DELETE），占位符使用 :name 形式
    :param params: 参数字典
    :param db_alias: 可选数据库别名，默认使用 default_db
    :param db_cfg: 可选；若传入（如 API 服务端从后台解析）则直接使用
    :param config_path: 可选；显式指定 config.yaml 路径
    :return: 影响行数
    """
